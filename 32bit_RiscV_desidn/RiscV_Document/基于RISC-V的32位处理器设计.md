...
控制与状态寄存器指令用于控制CPU的状态。在RISC-V中，只有一个CSR寄存器，用于控制CPU的状态。

CSR控制状态寄存器的地址是固定的，不同的CSR寄存器有不同的功能。CSR主要在特权架构中使用。
...
特权架构主要依靠CSR寄存器来实现。CSR寄存器是一种特殊的寄存器，用于控制CPU的状态。CSR寄存器的地址是固定的，不同的CSR寄存器有不同的功能。

# 2-RISC-V的FPGA处理器设计

在本项目中，使用的FPGA是硬木课堂Xilinx Artix-7 FPGA板，芯片型号是 Xilinx Artix-7 XC7A75T(Vivado代号xc7a75tfgg484-2)。具有电平开关、LED、键盘、数码管等基本外设。同时还有16位DDR3L共计128MB的片外内存。
