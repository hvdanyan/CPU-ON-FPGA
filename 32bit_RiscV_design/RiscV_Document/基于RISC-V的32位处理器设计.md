...
在本项目中，使用的FPGA是硬木课堂Xilinx Artix-7 FPGA板，芯片型号已在上方指出，为Xilinx Artix-7 XC7A75T。具有电平开关、LED、键盘、数码管等基本外设。同时还有16位DDR3L共计128MB的片外内存。
...
在进行CPU的设计前，首先需要了解计算机的组成原理及CPU的基本结构。请参考[3.0 处理器原理概述](#30-处理器原理概述)部分。
